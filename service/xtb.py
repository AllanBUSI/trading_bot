...
class XTB:
    ...
    def get_Candles(self, period, symbol, days=0, hours=0, minutes=0, qty_candles=0):
        ...
        for i in range(start_qty, qty):
            ...
            candle["open"]=result["returnData"]["rateInfos"][i]["open"]/1000
            candle["close"]=result["returnData"]["rateInfos"][i]["close"]/1000
            candle["high"]=result["returnData"]["rateInfos"][i]["high"]/1000
            candle["low"]=result["returnData"]["rateInfos"][i]["low"]/1000
            candles.append(candle)
        if len(candles)==1:
            return False
        return candles
    ...
    def make_Trade(self, symbol, cmd, transaction_type, volume, comment="", order=0, sl=0, tp=0, days=0, hours=0, minutes=0):
        print(symbol, cmd, transaction_type, volume, comment, sl, tp)
